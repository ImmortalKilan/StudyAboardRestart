...
每次 push 包含内容变更（JS / JSON / CSS / 图片）时，**必须同时修改以下三处版本号**（保持一致）：

1. `sw.js` 第 3 行：`const CACHE_VER = 'sasr-3.0';`
2. `index.html` SW 注册代码中：`const LOCAL_VER = '3.0';`
3. `version.json`：`{"version":"3.0"}`

版本号规则（semver-lite）：
- **小改动**（bug 修复、文案微调、小功能补丁）→ 递增小版本：`3.0` → `3.1` → `3.2`
- **大改动**（新模块、大型重构、UI 大改）→ 递增大版本：`3.x` → `4.0`

更新检测机制：页面启动时用 cache-busting 请求 `version.json`，与 `LOCAL_VER` 对比。版本不一致时触发 SW 更新；若浏览器缓存了 sw.js 导致 `reg.update()` 无效，会兜底弹出更新栏，点击后清空所有缓存并强制刷新。
